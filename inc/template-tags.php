...
if ( ! function_exists( 'shelly_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function shelly_posted_on() {
...
if ( ! function_exists( 'shelly_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function shelly_posted_by() {
		$byline = sprintf(
		/* translators: %s: post author. */
			esc_html_x( 'by %s', 'post author', 'shelly' ),
...
if ( ! function_exists( 'shelly_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function shelly_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'shelly' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'shelly' ) . '</span>',
					$categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'shelly' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'shelly' ) . '</span>',
			...
			comments_popup_link(
				sprintf(
					...
					/* translators: %s: post title */
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'shelly' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
if ( ! function_exists( 'shelly_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function shelly_post_thumbnail() {
...
if ( ! function_exists( 'shelly_header_nav' ) ) {
	/**
	 * Show header nav
	 */
	function shelly_header_nav() {
		$shelly_header_menu_args = array(
			'location'   => 'menu-1',
...
if ( ! function_exists( 'shelly_show_header_icons' ) ) {
	...
	function shelly_show_header_icons() {
		...
		$shelly_allowed_tags  = array_merge( $shelly_kses_defaults, $shelly_svg_args );

		if ( get_theme_mod( 'cart_header',
				true ) && shelly_is_woocommerce_activated() || get_theme_mod( 'search_header',
				true ) || get_theme_mod( 'profile_header', true ) && shelly_is_woocommerce_activated() ) {
		...
		if ( get_theme_mod( 'search_header', true ) ) {
			echo wp_kses( $shelly_search_icon, $shelly_allowed_tags );
		}
		if ( get_theme_mod( 'cart_header', true ) && shelly_is_woocommerce_activated() ) {
			echo wp_kses( sprintf( '<a class="c-header__link" href="%s" aria-label="%s">',
				esc_url( wc_get_cart_url() ),
				__( 'Cart', 'shelly' ) ),
				$shelly_allowed_tags );
			echo wp_kses( $shelly_cart_icon, $shelly_allowed_tags );
			echo wp_kses( "</a>", $shelly_allowed_tags );
		}
		if ( get_theme_mod( 'profile_header', true ) && shelly_is_woocommerce_activated() ) {
			echo wp_kses( sprintf( '<a class="c-header__link" href="%s" aria-label="%s">',
				esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ),
				__( 'Account', 'shelly' ) ),
				$shelly_allowed_tags );
			echo wp_kses( $shelly_user_icon, $shelly_allowed_tags );
			echo wp_kses( "</a>", $shelly_allowed_tags );
		}
...
if ( ! function_exists( 'shelly_post_categories' ) ) {
	/**
	 * Show post categories
	 */
	function shelly_post_categories() {
...
if ( ! function_exists( 'shelly_separator' ) ) {
	/**
 * Dash separator
 */
function shelly_separator( $condition = true ) {
	if ( $condition ) {
...
if ( ! function_exists( 'shelly__share_links' ) ) {
	function shelly__share_links() {
		if ( get_theme_mod( 'show_share_icons', true ) ) {
			$shelly_linkedin_url = "https://www.linkedin.com/shareArticle?mini=true&url=" . get_permalink() . "&title=" . get_the_title();
			$shelly_twitter_url  = "https://twitter.com/intent/tweet?url=" . get_permalink() . "&title=" . get_the_title();
			$shelly_facebook_url = "https://www.facebook.com/sharer.php?u=" . get_permalink();
...
if ( ! function_exists( 'shelly_posts_pagination' ) ) :
	/**
	 * Generate Posts Pagination
	 */
	function shelly_posts_pagination() {
